...
public class MeshData {
	...
	public float[] getFaceNormals() {
		...
		for (Face f : faceTable) {
			...
			v.scale(1 / (float)verts.size()); // unnecessary since you normalize after
			v.normalize();
	...
	public void loop() {
		...
		for(Edge edge : edgeTable)
		{
			...
			Vertex edgeVertex2 = vertexTable.get(edge.v2);
			Integer edgeInt1 = vertexTable.indexOf(edgeVertex1);
			Integer edgeInt2 = vertexTable.indexOf(edgeVertex2);
			ArrayList<Integer> faceNeighbouringVertices = new ArrayList<Integer>();
			faceNeighbouringVertices.addAll(findVertices(edge.f1));
			faceNeighbouringVertices.addAll(findVertices(edge.f2));
			if(faceNeighbouringVertices.size()!=6)
			{
				System.out.println("not 6 (double) face-neighboring vertices");
			}
			
			Vector3f newPosition = new Vector3f();
			Vector3f temp = new Vector3f();
			temp.scale(3f, edgeVertex1.position);
			newPosition.add(new Vector3f(temp));
			temp.scale(3f, edgeVertex2.position);
			newPosition.add(new Vector3f(temp));
			
			for(Integer i : faceNeighbouringVertices)
			{
				if(!i.equals(edgeInt1) && !i.equals(edgeInt2)){
					Vertex tempVertex = vertexTable.get(i);
					Vector3f tempPosition = new Vector3f(tempVertex.position);
					newPosition.add(tempPosition);
				}
			}
		...
		if(3*faceTable.size()!=vertexData.getIndices().length){
			System.out.println("3*faces!=indices");
		}
		// list of indices
		int[] indices = new int[4*3*faceTable.size()];
		for(int j=0; j<faceTable.size(); j++)
		{
			Face face = faceTable.get(j);
			// get Edges of face in counterclockwise order
			List<Edge> edges = findEdges(face);
			int nrEdges = edges.size();
			if(nrEdges!=3)
				System.out.println("not 3 edges in this face");
			int[] edgesIndex = new int[3];
			...
			for(int i=0; i<nrEdges; i++)
			{
				...
				int edgeIndex = edgesIndex[i];
				int newEdgeVertexIndex = edgeIndex+nrOldVertices;
				int previousEdgeIndex = edgesIndex[Math.floorMod(i-1, 3)];
				int newPreviousEdgeVertexIndex = previousEdgeIndex+nrOldVertices;
				int startVertexIndex = edge.getStartVertex(face);
				indices[4*3*j+3*i] = startVertexIndex;
				indices[4*3*j+3*i+1] = newEdgeVertexIndex;
				indices[4*3*j+3*i+2] = newPreviousEdgeVertexIndex;				
			}
			indices[4*3*j+3*3] = edgesIndex[0]+nrOldVertices;
			indices[4*3*j+3*3+1] = edgesIndex[1]+nrOldVertices;
